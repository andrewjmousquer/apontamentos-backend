...
import com.portal.security.JwtUserFactory;
...
public class UserService implements IUserService {
	...
	public Optional<UserModel> findLogin(UserModel model) throws AppException, BusException {
		...
		if (user.isPresent()) {
			if (user.get().getAccessList() != null) {
				user.get().getAccessList()
						.setMenus(this.accesslistMenuService.listMenuByAccessList(user.get().getAccessList().getId()));
				user.get().getAccessList().setCheckpoints(this.accessListCheckPointService
						.listCheckpointByAccessList(null, user.get().getAccessList().getId()));
			}
	...
	public Optional<UserModel> getById(Integer id) throws AppException, BusException {
		...
		if (userModel.isPresent()) {
			if (userModel.get().getPerson() != null) {
				userModel.get().setContacts(this.contactService.findByPerson(userModel.get().getPerson().getId()));
			}

			if (userModel.get().getAccessList() != null) {
				userModel.get().getAccessList().setMenus(
						this.accesslistMenuService.listMenuByAccessList(userModel.get().getAccessList().getId()));
			}
	...
	@Override
	public List<UserModel> listUsers() throws AppException, BusException {
		List<UserModel> userList = this.dao.list();
		return userList;
	}

	@Override
	public List<UserModel> listUsersAvalibleByTask(Integer stageID, Integer taskID) throws AppException, BusException {
		List<UserModel> userList = this.dao.listUsersAvalibleByStage(stageID, taskID);
		return userList;
	}
	...
	@Override
	public Optional<UserModel> saveOrUpdate(UserModel model, UserProfileDTO userProfile)
	...
	public Optional<UserModel> save(UserModel model, UserProfileDTO userProfile) throws AppException, BusException {
		...
		if (username.isPresent()) {
			if (username.get() != null && model.getUsername().trim().equals(username.get().getUsername().trim())) {
				throw new BusException(this.messageSource.getMessage("error.user.usernameExists", null,
		...
		model.setHashQRCode(PortalStringUtils.generateRandomAlphaNumericPassword(100));

		// Set CONTATOS
		if (!CollectionUtils.isEmpty(model.getContacts())) {
			model.getPerson().setContacts(new LinkedList<Contact>());
			model.getPerson().getContacts().addAll(model.getContacts());
		}
		...
		// Save CUSTOMER
		this.userCustomerService.saveUserCustomer(userSaved.get().getId(),
				(model.getCustomers() != null ? model.getCustomers() : null));
	...
	public Optional<UserModel> update(UserModel model, UserProfileDTO userProfile) throws AppException, BusException {
		...
		if (userModel.isPresent()) {
			Optional<PersonModel> personModel = this.personService.getById(userModel.get().getPerson().getId());
			if (personModel.isPresent()) {
				if (model.getChangePass()) {
					if (!model.getPassword().equals(userModel.get().getPassword())) {
						...
						userModel.get().setChangePass(false);
	...
	public Optional<UserModel> updateLoginData(String username) throws AppException, BusException {
		...
		if (user.isPresent()) {
			...
			Optional<ParameterModel> errorCountParameter = this.parameterService
					.find(new ParameterModel("TOTAL_ERROR_LOGIN"));
	...
	@Override
	public Optional<UserModel> updateLoginDataQR(UserModel qrCode) throws AppException, BusException {
		UserModel toSearch = new UserModel();
		toSearch.setHashQRCode(qrCode.getHashQRCode());
		Optional<UserModel> user = this.find(toSearch);
		if (user.isPresent()) {
			int passErrorCount = 0;
			Optional<ParameterModel> errorCountParameter = this.parameterService
					.find(new ParameterModel("TOTAL_ERROR_LOGIN"));
	...
	@Override
	public Optional<UserModel> changePassword(UserModel model, UserProfileDTO userProfile)
			throws AppException, BusException {
		try {
			this.validatePasswordPattern(model.getPassword());
	...
	@Override
	public void audit(UserModel model, AuditOperationType operationType, UserProfileDTO userProfile)
			throws AppException, BusException {
		try {
			String details = "{}";
		...
		if (passPattern != null) {
			...
			if (!matcher.matches()) {
				throw new BusException(
						this.messageSource.getMessage("error.user.passPattern", null, LocaleContextHolder.getLocale()));
			}
		}
	}
	...
	public void deleteUserCustomer(final Integer usrId, final List<CustomerModel> list)
			throws AppException, BusException {
	...
	private void checkPasswordHistory(UserModel user) throws AppException, BusException {
		if (user != null) {
			...
			if (hists != null) {
				if (hists.stream().filter(
						passHistModel -> passwordEncoder.matches(user.getPassword(), passHistModel.getPassword()))
						.findFirst().isPresent())
					throw new BusException(this.messageSource.getMessage("error.user.repeatedPass",
							new Object[] { passHistNum }, LocaleContextHolder.getLocale()));
			}
		}
	}
	...
	@Override
	public List<UserModel> findByTeam(Integer id) throws AppException, BusException {
		return this.dao.findByTeam(id);
	}

	@Override
	public UserDetails loadUserByQrCode(UserModel qrCode) throws UsernameNotFoundException, AppException {
		UserModel userSearch = new UserModel();
		userSearch.setHashQRCode(qrCode.getHashQRCode());

		Optional<UserModel> model = null;
		try {
			model = this.find(userSearch);
		...
		throw new UsernameNotFoundException(
				messageSource.getMessage("error.authentication.usernotfound", null, LocaleContextHolder.getLocale()));

	}

	@Override
	public Optional<UserModel> getUserAndOffice(@Valid String qrCode) throws AppException {
		return this.dao.getUserAndOffice(qrCode);
	}
