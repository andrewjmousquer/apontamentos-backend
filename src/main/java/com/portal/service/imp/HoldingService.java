...
public class HoldingService implements IHoldingService {
	...
	public Optional<HoldingModel> getById(Integer id) throws AppException, BusException {
		...
		if(holding.isPresent()) {
			...
			Optional<PersonModel> person = this.personService.getById(holding.get().getPerson().getId());
			if(person.isPresent()) {
				holding.get().setPerson(person.get());
				
			}
			Optional<ClassifierModel> type = this.classifierService.getById(holding.get().getType().getId());
			if(type.isPresent()) {
	...
	public Optional<HoldingModel> save(HoldingModel model, UserProfileDTO userProfile) throws AppException, BusException {
		...
		if(model.getPerson() != null && model.getPerson().getName() != null && !model.getPerson().getName().equals("")) {
