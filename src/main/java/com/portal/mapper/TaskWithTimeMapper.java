...
public class TaskWithTimeMapper implements ResultSetExtractor<List<TaskModel>> {
	...
	public List<TaskModel> extractData(ResultSet rs) throws SQLException, DataAccessException {
		...
		if (rs != null) {
			while (rs.next()) {
				...
				if (StringUtils.isNotEmpty(rs.getString("total_time")))
					model.setTotalTime(rs.getString("total_time"));

				if (StringUtils.isNotEmpty(rs.getString("checklist_file")))
					model.setCheckListFile(rs.getString("checklist_file"));
