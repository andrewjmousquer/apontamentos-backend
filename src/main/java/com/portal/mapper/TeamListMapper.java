...
public class TeamListMapper implements ResultSetExtractor<List<TeamModel>> {
	...
	public List<TeamModel> extractData(ResultSet rs) throws SQLException, DataAccessException {
		...
		if (rs != null) {
			while (rs.next()) {
				...
				model.setNumberOfUsers(rs.getInt("numberOfUsers"));
				model.setNumberOfStages(rs.getInt("numberOfstages"));
				list.add(model);
			}
		}
