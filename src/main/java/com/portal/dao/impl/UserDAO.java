...
public class UserDAO extends BaseDAO implements IUserDAO {
	...
	public Optional<UserModel> find(UserModel model) throws AppException {
		...
		try {
			...
			if (model.getPerson() != null && model.getPerson().getName() != null) {
				...
				params.addValue("name", this.mapLike(model.getPerson().getName()));
			}
			...
			if (model.getHashQRCode() != null && model.getHashQRCode() != "") {
				query.append("AND u.hash_qr_code LIKE :qrCode ");
				params.addValue("qrCode", model.getHashQRCode());
			}
			...
			logger.error(e.getMessage());
			throw new AppException(
					this.messageSource.getMessage("error.user.usernameExists", null, LocaleContextHolder.getLocale()));
		}
	...
	public List<UserModel> list() throws AppException {
		...
		try {
			...
			log.trace("[QUERY] user.findByTeam: {} [PARAMS] : {}", query);
	...
	public List<UserModel> search(UserModel model) throws AppException {
		...
		try {
			...
			if (model.getUserType() != null && model.getUserType().getId() != null) {
				query.append("AND u.type_cla = :type ");
				params.addValue("type", model.getUserType().getId());
			}
	...
	public Optional<UserModel> save(UserModel model) throws AppException {
		...
		params.addValue("hash_qr_code", model.getHashQRCode());
	...
	public Optional<UserModel> update(UserModel model) throws AppException {
		try {
			...
			query.append("hash_qr_code = :hash_qr_code,");
	...
	public List<UserModel> findByTeam(Integer id) throws AppException {
		...
		try {
			...
			query.append("INNER JOIN " + schemaName + "classifier as ut on ut.cla_id = u.type_cla ");
			query.append("LEFT JOIN " + schemaName + "customer as c ON c.cus_id = u.cus_id ");
			query.append("LEFT JOIN user_team ");
			query.append("ON u.usr_id = user_team.usr_id ");
			query.append("WHERE user_team.tam_id= :id");

			MapSqlParameterSource params = new MapSqlParameterSource();
			params.addValue("id", id);

			log.trace("[QUERY] user.findByTeam: {} [PARAMS] : {}", query, params.getValues());

			List<UserModel> users = this.getJdbcTemplatePortal().query(query.toString(), params, new UserMapper());
	...
	@Override
	public List<UserModel> listUsersAvalibleByStage(Integer stageID, Integer taskID) throws AppException {
		List<UserModel> listReturn = null;
		try {
			StringBuilder query = new StringBuilder();
			query.append("SELECT ");
			...
			query.append("RIGHT JOIN " + schemaName + "user_team AS ute ON ute.usr_id = u.usr_id ");
			query.append("RIGHT JOIN " + schemaName + "team AS t ON ute.tam_id = t.tam_id ");
			query.append("RIGHT JOIN " + schemaName + "stage_team AS st ON st.tam_id = t.tam_id ");
			query.append(
					"WHERE st.stg_id = :stageID AND NOT EXISTS (SELECT 1 FROM task_user AS tu WHERE tu.usr_id = u.usr_id AND tu.tsk_id = :taskID ) ");
			query.append("GROUP BY u.usr_id ");
			query.append("ORDER BY username ");

			log.trace("[QUERY] user.listUsersAvalibleByStage: {} [PARAMS] : {}", query);
	...
	public Optional<UserModel> getUserAndOffice(@Valid String qrCode) throws AppException {
		...
		try {
			...
			query.append("LEFT JOIN " + schemaName + "user_customer uc ON u.usr_id = uc.usr_id ");
			query.append("LEFT JOIN " + schemaName + "menu as m ON a.mnu_id = m.mnu_id ");
			query.append("WHERE hash_qr_code=:hashCode");

			MapSqlParameterSource params = new MapSqlParameterSource();
			params.addValue("hashCode", qrCode);

			List<UserModel> user = this.getJdbcTemplatePortal().query(query.toString(), params, new UserMapper());
			if (!CollectionUtils.isEmpty(user)) {
				objReturn = Optional.ofNullable(user.get(0));
			}
